...
import inboxAction from '../../actions/inboxAction';
...
function Inbox(props) {
    ...
    const handleRequest = (from = selectFromTime, to = selectToTime) => {
        ...
        axios.post('https://logger-api.antexpert.uz/find_by_log_ts_and_status_id', {
            ...
            // props.inboxAction(r.data.payload);
    ...
    const handleArchivePostClick = () => {
        ...
        axios.post('https://logger-api.antexpert.uz/change_status', {
            ...
            if (r.status === 200) {
                ...
                const ddata = fullData.filter(el => !selected.includes(el.id));
                console.log(ddata);

                setFullData(ddata);
                const ddataa = mainData.filter(el => !selected.includes(el.id));
                console.log(ddataa);
        ...
        <Route render={props =>
            (Cookie.get('cookies')) ? (
                <div className={classes.root}>
                    <Grid container spacing={1} className="app_instance" justify="space-between" alignItems="flex-start">
                        <Grid item xs={2}>
                            ...
                            <Grid container spacing={1}>
                                <Grid item xs={4}>
                                    <Button className={classes.button} variant="contained" disabled onClick={handleInboxPostClick} >Inb</Button>
                                </Grid>
                                <Grid item xs={4}>
                                    <Button className={classes.button} variant="contained" onClick={handleArchivePostClick} >Arc</Button>
                                </Grid>
                                <Grid item xs={4}>
                                    <Button className={classes.button} variant="contained" onClick={handleProcessingPostClick} >Pro</Button>
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item xs={6}>
